...
mulh a b =
  let res = fromIntegral (fromIntegral a :: Int32) * fromIntegral (fromIntegral b :: Int32) :: Int64
   in fromIntegral (res `shiftR` 32)

mulhsu :: Word32 -> Word32 -> Word32
mulhsu a b =
  let res = fromIntegral (fromIntegral a :: Int32) * fromIntegral b :: Int64
   in fromIntegral (res `shiftR` 32)

mulhu :: Word32 -> Word32 -> Word32
mulhu a b =
  let res = fromIntegral a * fromIntegral b :: Word64
   in fromIntegral (res `shiftR` 32)
...
executeSystem (Trap ECALL) = do
  currentPC <- getPC
  -- ecall is handled directly by the host kernel: we do NOT take an architectural trap,
  -- so the trap CSRs are deliberately left untouched here
  -- this might change when we add custom trap handlers in the *far* future
