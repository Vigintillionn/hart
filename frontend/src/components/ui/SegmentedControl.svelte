<script lang="ts" generics="T">
  ...
  const activeText = $derived(accent === "primary" ? "text-primary" : "text-text");
</script>

<div class="inline-flex gap-0.5 rounded-md border border-border bg-surface-0 p-0.5">
  {#each options as opt (String(opt.value))}
    <button
      class="font-semibold transition-colors {item} {value === opt.value
