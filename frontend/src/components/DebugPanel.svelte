...
<div class="flex flex-col h-full overflow-hidden">
  <div class="bg-zinc-950 px-3 py-1.5 text-xs uppercase tracking-wider text-zinc-500 border-b border-zinc-800 shrink-0">
    CPU State
  </div>
  ...
  {#if cpuStore.cpuState}
    <div class="flex-1 min-h-0">
      <PaneGroup direction="vertical" autoSaveId="debug-panel-layout">
        <Pane 
          defaultSize={50} minSize={10} collapsible={true} collapsedSize={0}
        ...
        >
          <div class="h-full overflow-y-auto p-4 min-h-0 bg-zinc-900">
            <CpuHeader pc={cpuStore.cpuState.pc} cycles={cpuStore.cpuState.cycles} />
            <RegisterFile registers={cpuStore.cpuState.regs} />
          </div>
        </Pane>

        <PaneResizer class="h-1 bg-zinc-800 hover:bg-zinc-600 data-[resize-handle-state=drag]:bg-sky-500 transition-colors cursor-row-resize relative z-10 flex items-center justify-center">
          <div class="absolute inset-x-0 -top-1.5 -bottom-1.5" ondblclick={() => layoutStore.memoryPaneRef?.isCollapsed() ? layoutStore.memoryPaneRef?.expand() : layoutStore.memoryPaneRef?.collapse()}></div>
        </PaneResizer>
  ...
  {:else}
    <div class="flex items-center justify-center h-32 text-zinc-600 italic text-sm flex-1">
